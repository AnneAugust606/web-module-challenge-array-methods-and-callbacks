...
const finals2014 = fifaData.filter((stuff) => {
  return stuff.Year === 2014 && stuff.Stage === "Final";
});
...
function getFinals(data) {
    const finals = data.filter((stuff)=> {
        return stuff.Stage === 'Final';
    })
    return finals;
 }
...
function getWinnersByYear(data, finalsCB, getYearsCB, getWinCB) {
    const win = getWinCB(data, finalsCB);
    const years = getYearsCB(data, finalsCB);
    return win.map((item, index) => `In ${years[index]}, ${item} won the world cup!`);
}
...
function getAverageGoals(data) {
    const homeaverage = data.reduce((acc, item) => {
return acc + item['Home Team Goals']+ item['Away Team Goals'];
    }, 0)
    return (homeaverage / data.length).toFixed(2);
 }
